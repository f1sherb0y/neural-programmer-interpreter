...
from modern_npi.constants import Action, Direction, Pointer


def _digits_lsd_first(value: str) -> list[int]:
...
def decimal_add(first: str, second: str) -> str:
    first_digits = _digits_lsd_first(first)
    second_digits = _digits_lsd_first(second)
    carry = 0
    output = []
...
class AdditionEnvironment:
    """Four-row scratchpad with one pointer per row.

    Internally, index zero is the rightmost (least-significant) column. Moving
    LEFT therefore increments a pointer, matching the paper's visual layout.
    """
    ...
    def execute(self, arguments: tuple[int, int, int]) -> None:
